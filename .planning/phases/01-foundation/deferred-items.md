...
## Untracked `supabase/.temp/` not gitignored (found during 01-04)

- **Path:** `supabase/.temp/` (Supabase CLI runtime scratch: `cli-latest`, `project-ref`, `pooler-url`, version pins, etc.)
- **Issue:** Generated by the local Supabase CLI; appears as untracked in `git status` and is not listed in `.gitignore`.
- **Why deferred:** Not part of 01-04's file set; unrelated to the deep-dive UI. Left unstaged (per-file staging), never committed.
- **Suggested fix (future):** add `supabase/.temp/` to `.gitignore`.
