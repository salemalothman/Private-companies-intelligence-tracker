...
/**
 * Cumulative invested capital over time — one point per investment date. REAL
 * data only (each point is the running sum of recorded investments up to that
 * date); powers the dashboard sparklines and the hero chart's comparison line.
 * Returns [] when nothing is dated, never a fabricated shape.
 */
export function investedCapitalSeries(
  companies: CompanyWithRelations[],
): PortfolioValuePoint[] {
  const events = companies
    .flatMap((c) => c.investments)
    .filter((i) => i.investment_date && i.amount != null)
    .sort(
      (a, b) =>
        new Date(a.investment_date).getTime() -
        new Date(b.investment_date).getTime(),
  ...
  let running = 0;
  return events.map((i) => {
    running += i.amount as number;
    return { date: i.investment_date, value: running };
  });
}
